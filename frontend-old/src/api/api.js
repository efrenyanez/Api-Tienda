...
const api = {
  ...
  guardarProducto: async (formData) => {
    try {
      ...
      if (!res.ok) {
        // Intentar leer el cuerpo de error devuelto por el servidor para mostrar mensaje claro
        let errMsg = 'Error al guardar producto';
        try {
          const errBody = await res.json();
          if (errBody && errBody.message) errMsg = errBody.message;
          else if (errBody && errBody.status) errMsg = JSON.stringify(errBody);
        } catch (parseErr) {
